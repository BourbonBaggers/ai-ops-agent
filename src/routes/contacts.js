...
async function createContact(request, env) {
  // throws 400 on missing/invalid body or wrong content-type
...
async function updateContact(id, request, env) {
  ...
  const exists = await env.DB.prepare(`SELECT id FROM contacts WHERE id = ?`)
    .bind(id)
    .first();
  if (!exists) return json({ status: "error", message: "not found" }, 404);

  // Be consistent: require JSON body for PUT too (and make failures 400 not 500)
  const body = await requireJsonBody(request);
...
async function importContacts(request, env) {
  ...
  if (!ct.includes("multipart/form-data")) {
    return json(
      { status: "error", message: "Expected multipart/form-data with field 'file'." },
      400
    );
  }
  ...
  if (!file || typeof file === "string") {
    return json(
      { status: "error", message: "Missing file. Add form-data field named 'file'." },
      400
    );
  }
  ...
  if (!stmts.length) {
    return json({ status: "error", message: "No valid rows (missing emails)." }, 400);
  }
